...
export default function CheckoutPage() {
 ...
 const GlassCard: React.FC<React.PropsWithChildren<{ className?: string }>> = ({ className = '', children }) => (
  <div className={`rounded-2xl glass-card-lg ring-1 ring-white/10 ${className}`}>
   {children}
  </div>
 );
        ...
        <div key={step} className="flex items-center">
         <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium border-2 transition-all duration-300 ${
          currentStep >= step 
           ...
           : 'bg-stone-600 text-slate-300 border-stone-500'
         }`}>
          {step}
         </div>
         {step < 3 && (
          <div className={`w-16 h-1 mx-2 rounded-full transition-all duration-300 ${
           currentStep > step ? 'bg-lime-400' : 'bg-stone-500'
          }`} />
         )}
        </div>
       ))}
           ...
           <div key="shipping-form-grid" className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div key="firstName">
             ...
             <input
              ...
              className={errors.firstName 
               ...
               : 'w-full rounded-lg border border-stone-600 bg-black/40 px-3 py-2.5 text-white outline-none ring-0 transition focus:border-lime-300/60 focus:ring-2 focus:ring-lime-400/50'
              }
             />
            ...
            <div key="company" className="md:col-span-2">
             ...
             <input
              ...
              className="w-full rounded-lg border border-stone-600 bg-black/40 px-3 py-2.5 text-white outline-none ring-0 transition focus:border-lime-300/60 focus:ring-2 focus:ring-lime-400/50"
             />
            </div>
         ...
         <div className="space-y-8">
             ...
             <input
              ...
              className={`w-full rounded-lg border bg-black/40 px-3 py-2.5 text-white outline-none ring-0 transition focus:ring-2 placeholder:text-slate-400 ${
               errors.cardNumber 
                ...
                : 'border-stone-600 focus:border-lime-300/60 focus:ring-lime-400/50'
              }`}
             />
          ...
          <div className="mt-8 flex space-x-4">
           <button
            type="button"
            onClick={handleBack}
            className="flex-1 rounded-full border border-stone-600 bg-stone-700 px-6 py-3 font-medium text-white transition hover:bg-stone-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-lime-400/60"
           >
            Back
           </button>
           <button
         ...
         <div className="rounded-lg glass-card-sm p-4">
          ...
          <div className="space-y-4">
           {cart.items.map((item) => {
            ...
            return (
             <div key={item.id} className="border-b border-stone-600 pb-4 last:border-b-0 last:pb-0">
