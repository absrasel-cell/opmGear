 ...
 if (baseUnitPrice && baseUnitPrice > unitPrice) {
  ...
  return (
   <div className="text-right">
    ...
    <div className="mt-1 p-2 glass-alert-success rounded-md">
...
export default function CartPage() {
 ...
 const GlassCard: React.FC<React.PropsWithChildren<{ className?: string }>> = ({ className = '', children }) => (
  <div className={`rounded-2xl glass-button ring-1 ring-stone-700 ${className}`}>
   {children}
  </div>
 );
     ...
     <GlassCard className="px-5 py-4">
      <div className="flex items-center justify-between">
       ...
       <div className="flex gap-2">
        ...
        <button
         onClick={() => {
          // Get the last added item (most recent) to restore its configuration
          const lastItem = cart.items[cart.items.length - 1];
          if (lastItem) {
           // Store the cart item configuration in sessionStorage to restore it on the product page
           sessionStorage.setItem('restore_cart_item_config', JSON.stringify({
            selectedColors: lastItem.selectedColors,
            logoSetupSelections: lastItem.logoSetupSelections,
            multiSelectOptions: lastItem.multiSelectOptions,
            selectedOptions: lastItem.selectedOptions,
            additionalInstructions: lastItem.additionalInstructions,
            priceTier: lastItem.priceTier || 'Tier 1'
           }));
           
           // Navigate to the product page with the slug from the last item
           if (lastItem.productSlug) {
            router.push(`/customize/${lastItem.productSlug}`);
           } else {
            // If no slug available, go to store to select a product
            router.push('/store');
           }
          } else {
           // If no items, just go to the store
           router.push('/store');
          }
         }}
                ...
                <div key={colorName} className="rounded-lg glass-morphism-subtle p-4">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {sizeEntries.map(([size, quantity]: any) => {
                   ...
                   return (
                    <div key={`${colorName}-${size}`} className="rounded-lg glass-card-sm p-3">
              ...
              <div className="rounded-xl glass-card-md p-4 md:p-5 ring-1 ring-white/10">
               <div className="mb-3 flex items-center justify-between">
                ...
                {isCalculatingCosts ? (
                 ...
                 <span className="rounded-full glass-badge px-2 py-1 text-xs text-lime-200">Additional Cost</span>
                )}
               </div>
               ...
               <div className="space-y-3">
                {itemCostBreakdowns[item.id]
                 ? itemCostBreakdowns[item.id].logoSetupCosts.map((logoCost, index) => (
                   <div key={index} className="flex items-center justify-between rounded-lg glass-card-sm p-3">
                    <div>
                 ...
                 : Object.entries(item.logoSetupSelections).map(([logoKey, logoConfig]: any) => (
                   <div key={logoKey} className="flex items-center justify-between rounded-lg glass-card-sm p-3">
                    <div>
              ...
              <div className="rounded-xl glass-card-md p-4 md:p-5 ring-1 ring-white/10">
               <div className="mb-3 flex items-center justify-between">
                ...
                {isCalculatingCosts ? (
                 ...
                 <span className="rounded-full glass-badge px-2 py-1 text-xs text-purple-200">Additional Cost</span>
                )}
               </div>
               <div className="space-y-3">
                {itemCostBreakdowns[item.id]
                 ? itemCostBreakdowns[item.id].accessoriesCosts.map((acc, i) => (
                   <div key={i} className="flex items-center justify-between rounded-lg glass-card-sm p-3">
                    <div>
              ...
              <div className="rounded-xl glass-card-md p-4 md:p-5 ring-1 ring-white/10">
               <div className="mb-3 flex items-center justify-between">
                ...
                {isCalculatingCosts ? (
                 ...
                 <span className="rounded-full glass-badge px-2 py-1 text-xs text-purple-200">Premium Cost</span>
                )}
               </div>
              ...
              <div className="rounded-xl glass-card-md p-4 md:p-5 ring-1 ring-white/10">
               <div className="mb-3 flex items-center justify-between">
                ...
                {isCalculatingCosts ? (
                 ...
                 <span className="rounded-full glass-badge px-2 py-1 text-xs text-orange-200">Additional Cost</span>
                )}
               </div>
               ...
               <div className="space-y-3">
                {itemCostBreakdowns[item.id] && itemCostBreakdowns[item.id].deliveryCosts.length > 0 ? (
                 ...
                 <div className="flex items-center justify-between rounded-lg glass-card-sm p-3">
                  <div>
               ...
               <div className="mb-3 flex items-center justify-between">
                ...
                {isCalculatingCosts ? (
                 ...
                 <div className="text-right">
                  ...
                  <div className="text-xs text-cyan-200/80">Total Closures Cost</div>
                 </div>
                ) : (
                 <span className="rounded-full glass-badge px-2 py-1 text-xs text-cyan-200">Additional Cost</span>
                )}
               </div>
                 ...
                 itemCostBreakdowns[item.id].closureCosts.map((s, i) => (
                  <div key={i} className="flex items-center justify-between rounded-lg glass-card-sm p-3">
                   <div>
                    ...
                    <p className="text-xs text-slate-300">Premium closure option</p>
                   </div>
                   <DiscountedPriceDisplay
                    unitPrice={s.unitPrice}
                    totalUnits={itemCostBreakdowns[item.id].totalUnits}
              ...
              <div className="rounded-xl glass-card-md p-4 md:p-5 ring-1 ring-white/10">
               ...
               <div className="grid grid-cols-2 gap-3 md:gap-4">
                {item.selectedOptions['bill-shape'] && (
                 <div className="rounded-lg glass-card-sm p-3 text-center">
           ...
           <>
            ...
            {Object.values(itemCostBreakdowns).some((b) => b.servicesCosts && b.servicesCosts.length > 0) && (
             <div className="flex items-center justify-between rounded-lg p-2">
              ...
              <span className="text-sm font-semibold text-white">
               {formatPrice(
                Object.values(itemCostBreakdowns).reduce((sum, b) => sum + (b.servicesCosts || []).reduce((s, c) => s + c.cost, 0), 0)
               )}
              </span>
             </div>
            )}
