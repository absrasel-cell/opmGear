...
const formatCapColors = (frontCrown: string, backCrown: string, bill: string) => {
  const colors = [];
  if (frontCrown && frontCrown !== 'Not specified') colors.push(`Front: ${frontCrown}`);
  if (backCrown && backCrown !== 'Not specified') colors.push(`Back: ${backCrown}`);
  if (bill && bill !== 'Not specified') colors.push(`Bill: ${bill}`);
  return colors.length > 0 ? colors.join(', ') : 'Colors not specified';
};
...
Please provide a detailed quote with cost breakdown.`;
    ...
    // Call the original onGenerateQuote if provided
