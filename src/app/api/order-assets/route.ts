...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!user) {
      console.log('❌ [API] order-assets: No authenticated user');
      return NextResponse.json({ error: 'Unauthorized - Please log in again' }, { status: 401 });
    }

    console.log('✅ [API] order-assets: User authenticated:', user.id);

    const url = new URL(request.url);
    const orderId = url.searchParams.get('orderId');
    
    if (!orderId) {
      console.log('❌ [API] order-assets: Missing orderId parameter');
    ...
    console.log('✅ [API] order-assets: Order found:', { id: order.id, userId: order.userId, status: order.status });

    // Get user profile for role checking
    const userProfile = await getUserProfile(user.id);
    console.log('👤 [API] order-assets: User profile:', { 
      id: userProfile?.id, 
    ...
    console.log('🔐 [API] order-assets: Access check:', { isOwner, isAdmin, isMasterAdmin });

    if (!isOwner && !isAdmin) {
      console.log('❌ [API] order-assets: Access denied for user:', user.id);
    ...
    for (const asset of assets) {
      ...
      if (signedUrlError) {
        console.error('❌ [API] order-assets: Error creating signed URL for asset:', asset.id, signedUrlError);
        signedUrlErrors++;
        continue; // Skip this asset but don't fail the entire request
      }

      console.log(`✅ [API] order-assets: Signed URL created for asset ${asset.id}`);
    ...
    console.log(`✅ [API] order-assets: Successfully processed ${assetsWithSignedUrls.length} assets with ${signedUrlErrors} errors`);

    return NextResponse.json(assetsWithSignedUrls);

  } catch (error) {
    console.error('❌ [API] order-assets: Unexpected error:', error);
