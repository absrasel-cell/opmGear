...
export async function GET() {
 try {
  console.log('🏭 API: Fetching blank cap products...');
  ...
  console.log(`🏭 API: Retrieved ${sanityProducts.length} total products from Sanity`);
  ...
  const blankCapProducts = sanityProducts
   .filter((product: any) => {
    ...
    console.log(`🔍 API: Product "${product.name}" - Active: ${isActiveProduct}, Image: ${!!hasMainImage}, Slug: ${!!hasSlug}, Type: ${product._type}, ProductType: ${product.productType}`);
  ...
  console.log(`🏭 API: Final blank cap products: ${blankCapProducts.length}`);
  ...
  console.error('❌ API: Blank cap products error:', error);
  return NextResponse.json({
   success: false,
