...
export default function ProductClient({ product, productSlug, prefillOrderId, reorder = false }: ProductClientProps) {
 ...
 // Restore cart item configuration when coming back from cart page
 useEffect(() => {
  const restoreCartItemConfig = () => {
   if (typeof window === 'undefined') return;
   
   const storedConfig = sessionStorage.getItem('restore_cart_item_config');
   if (storedConfig) {
    try {
     const config = JSON.parse(storedConfig);
     
     // Restore all the configuration
     if (config.selectedColors) {
      setSelectedColors(config.selectedColors);
     }
     if (config.selectedOptions) {
      setSelectedOptions(config.selectedOptions);
     }
     if (config.logoSetupSelections) {
      setLogoSetupSelections(config.logoSetupSelections);
     }
     if (config.multiSelectOptions) {
      setMultiSelectOptions(config.multiSelectOptions);
     }
     if (config.additionalInstructions) {
      setAdditionalInstructions(config.additionalInstructions);
     }
     
     // Clear the stored configuration after restoring
     sessionStorage.removeItem('restore_cart_item_config');
     
     console.log('🔄 Restored cart item configuration from cart page');
    } catch (error) {
     console.error('Error restoring cart item configuration:', error);
    }
   }
  };
  
  // Only run once on component mount, not when dependencies change
  restoreCartItemConfig();
 ...
 const handleQuickSave = async () => {
  ...
  try {
   const updatePayload = {
    ...
    // Include logo file for upload during order update
    logoFile,
    ...
    // Include cost breakdown to preserve Premium Fabric costs and other calculated costs
 ...
 return (
  <div className="mx-auto relative px-3" style={{ maxWidth: '1850px' }}>
   ...
   <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
    ...
    <div className="absolute top-0 left-0 w-32 h-32 bg-gradient-to-br from-cyan-400/10 to-blue-900 rounded-full animate-float"></div>
          ...
          return (
           <div className="mt-4 text-center">
            ...
            >
             ...
             <span className="text-xs bg-stone-500 px-2 py-1 rounded-full">
      ...
      <div className="bg-white/80 backdrop-blur-xl shadow-xl rounded-xl p-6">
       <div className="space-y-4">
        ...
        {Object.keys(selectedColors).length > 0 && hasLeatherOrRubberPatchSelected() && (
         <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-2xl p-6 shadow-lg border border-amber-200 dark:border-amber-700 hover:shadow-xl transition-all duration-300 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
         ...
         <AddToCartButton
          ...
          productName={product.name}
          productSlug={productSlug || product.name.toLowerCase().replace(/\s+/g, '-')}
          priceTier={product.priceTier || 'Tier 1'}
          selectedColors={selectedColors}
          logoSetupSelections={logoSetupSelections}
          selectedOptions={{
